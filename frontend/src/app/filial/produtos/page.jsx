...
export default function Produtos() {
  ...
  const fetchProdutos = async () => {
    ...
    if (!userToken) {
      setMensagemFeedback({ type: 'error', text: 'Sessão expirada ou não autenticada. Faça login novamente.' });
      setIsLoading(false);
      return;
    }
    setIsLoading(true);
...
//  logica da submissao com o método post q o professor passou (POST)
  const handleSubmit = async (e) => {
    ...
    const produtoData = {
      ...
      descricao: descricao,
      preco_unitario: 0,
      validade: new Date().toISOString().split('T')[0], // Exemplo
          ...
          <form onSubmit={handleSubmit}>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="grid gap-4 py-4 max-h-[70vh] overflow-y-auto">
                {/* Inputs do Formulário... (omitidos para brevidade) */}
        ...
        <ul className="flex flex-wrap -mb-px">
          <li className="me-2">
            <a onClick={() => setAbaAtiva(ABAS.ATIVOS)} className={getAbaClasses(ABAS.ATIVOS)}>
              Produtos Ativos ({produtos.filter(p => { const v = p.validade ? new Date(p.validade) : null; return !v || v >= new Date(); }).length})
            </a>
          </li>
          <li className="me-2">
          ...
          <li className="me-2">
            <a onClick={() => setAbaAtiva(ABAS.VENCIDOS)} className={getAbaClasses(ABAS.VENCIDOS)}>
              Vencidos ({produtos.filter(p => { const v = p.validade ? new Date(p.validade) : null; return v && v < new Date(); }).length})
            </a>
          </li>
        </ul>
      </div>
